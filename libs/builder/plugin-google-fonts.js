GoogleFontsManager = {
	...
	updateFontList: function () {
		let googleFontsLink = $("#google-fonts-link", Rigel.Builder.frameHead);
		...
		if (googleFontsLink.length < 1) {
			...
			Rigel.Builder.frameHead.append(googleFontsLink);
		}
...
Rigel.FontsManager.addProvider("google", GoogleFontsManager);
	
let googleFonts = {};
let googlefontNames = [];
//load google fonts list and update wyswyg font selector and style tab font-family list
$.ajax({
	url: Rigel.baseUrl + "../../resources/google-fonts.json",
	cache: true,
	dataType: "json",
	success: function(data) {
		...
		let list = Rigel.Components.getProperty("_base", "font-family");
		list.onChange = function (node,value, input, component) {
			let option = input.options[input.selectedIndex];
			Rigel.FontsManager.addFont(option.dataset.provider, value, node);
			return node;
		};
		...
		Rigel.Components.updateProperty("_base", "font-family", {data:list.data});
